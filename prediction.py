...
def predict(model, data:Dataset, device:torch.device, compute_loss:bool=False, 
            normalization:str="batch", num_workers:int=0, test_cfg:Dict=None, temperature:int=None):
    """
    ...
    for batch_data in tqdm.tqdm(data_iter, desc="Validating"):
        ...
        if compute_loss: 
            ...
            with torch.no_grad():
                ...
                src_mask=src_mask, trg_mask=trg_mask, encoder_output=None, trg_truth=trg_truth, temperature=temperature)
    ...
    # valid_hypotheses = data.tokenizer[data.trg_language].decode(all_outputs)
