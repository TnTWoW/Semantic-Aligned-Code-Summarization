...
class Transformer(nn.Module):
    ...
    def __init__(self, encoder: TransformerEncoder, decoder:TransformerDecoder,
        ...
        self.proj = torch.nn.Parameter(torch.rand(self.model_dim, self.model_dim))
    ...
    def forward(self, return_type: str = None,
        ...
        if return_type == "loss":
            ...
            src_semantic = encode_output[:, 0]
            clt_loss, contrast_acc = self.get_ctl_loss_v2(src_semantic, trg_input, trg_mask.squeeze(),
    ...
    def get_ctl_loss_v2(self, src_embedding, trg_input, trg_mask, dynamic_coefficient=1, normalize=True,
                        temperature=0.02):

        # src_embedding = [batch_size, src_len, embed_size]
        trg_embedding = self.trg_embed(trg_input)
        trg_embedding = trg_embedding.mul((trg_mask).unsqueeze(-1)).sum(1).div(
            (trg_mask).unsqueeze(-1).sum(dim=1))
        ...
        return loss * batch_size, contrast_acc
