...
from graph_utils import partition_num
...
class RandomWalker:
    ...
    def simulate_walks(self, num_walks, walk_length, start_node=None, workers=1, verbose=0):
        if start_node == None:
            G = self.G
            nodes = list(G.nodes())
            terminal_nodes = []
            for node in nodes:
                if node[0] >= '0' and node[0] <= '9':
                    terminal_nodes.append(node)
            nodes = natsorted(terminal_nodes)
        else:
            nodes = start_node
        # nodes
        # terminal_nodes = []
        # for i in range(len(nodes) - 1):
        #     first_char = nodes[i][0]
        #     second_char = nodes[i + 1][0]
        #     if first_char >= '0' and first_char <= '9' and second_char < '0' or second_char > '9':
        #         terminal_nodes = nodes[:i+1]
        #         break
