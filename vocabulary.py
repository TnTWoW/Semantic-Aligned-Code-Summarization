...
class Vocabulary(object):
    ...
    def __init__(self, tokens: List[str]) -> None:
        ...
        print(self.pad_index)
        print(self.unk_index)
        print(self.bos_index)
...
class Custom_Vocabulary(object):
    """
    Vocabulary class mapping between tokens and indices.
    """
    ...
    def __init__(self, tokens: Dict[str, int]) -> None:
        ...
        self.specials = [UNK_TOKEN, PAD_TOKEN, BOS_TOKEN, EOS_TOKEN]
        self._stoi: Dict[str, int] = tokens  # string to index
        self._itos: List[str] = []  # index to string
    ...
    def log_vocab(self, number: int) -> str:
        "First how many number of tokens in Vocabulary."
        return " ".join(f"({token}) {id}" for token, id in list(self._stoi.items())[:number])
        # return " ".join(f"({token}) {id}" for token, id in enumerate(self._stoi[:number]))
