...
class CosineAnnealing():
    ...
    def get_temp(self):
        self.step += 1
        return 0.5 * (1 + math.cos(math.pi * self.step / self.T)) * self.init_lr
...
class TrainManager(object):
    ...
    def __init__(self, model: Transformer, cfg: dict) -> None:
        ...
        self.temperature = CosineAnnealing(T=epochs)
    ...
    def train_and_validate(self, train_data:Dataset, valid_data:Dataset) -> None:
        ...
        try:
            for epoch_no in range(self.epochs):
                ...
                epoch_loss = 0
                temperature = self.temperature.get_temp()

                for batch_data in self.train_iter:
                    batch_data.move2cuda(self.device)
                    normalized_batch_loss = self.train_step(batch_data, temperature)
                ...
                if (epoch_no + 1) % self.validation_freq == 0:
                    valid_duration_time = self.validate(valid_data, temperature)
    ...
    def train_step(self, batch_data ,temperature):
        """
        ...
        batch_loss = self.model(return_type="loss", src_input=src_input, trg_input=trg_input, src_mask=src_mask,
                                trg_mask=trg_mask, encoder_output=None, trg_truth=trg_truth, temperature=temperature)
    ...
    def validate(self, valid_data: Dataset, temperature):
        """
        ...
        (valid_scores, valid_references, valid_hypotheses, 
         valid_sentences_scores, valid_attention_scores) = predict(model=self.model, data=valid_data, device=self.device, 
                compute_loss=True, normalization=self.normalization, num_workers=self.num_workers, test_cfg=self.valid_cfg, temperature=temperature)
